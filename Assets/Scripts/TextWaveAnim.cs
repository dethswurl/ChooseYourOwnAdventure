...
namespace TMPro.Examples
{
    ...
    public class TextWaveAnim : MonoBehaviour
    {
        ...
        void Start()
        {
            startTargetRotation = Random.Range(0.25f, 0.25f * rotationRandomness);
            currentRotation = 0f;
        ...
        IEnumerator AnimateVertexColors()
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < lineCount; i++)
                {
                    ...
                    targetRotation = startTargetRotation * RotationMultiplier;
                    if (pause <= 0f)
                    {
                        if (rotationUp)
                        {
                            r = Mathf.Lerp(currentRotation, targetRotation, elapsedTime / 2f);
                        ...
                        rotation = Quaternion.Euler(0, r * 2f, r);
                        elapsedTime += SpeedMultiplier * Time.deltaTime;
                    }
                    else
                    {
                        pause -= 0.1f;
