...
namespace TMPro.Examples
{
    ...
    public class OldTextMove : MonoBehaviour
    {
        ...
        public float rotationRandomNess = 0f;
        ...
        void Start()
        {
            rotationUp = true;
            fRotation = 0f;
            rotation = Quaternion.Euler(0, 0, fRotation);
        ...
        IEnumerator AnimateVertexColors()
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < lineCount; i++)
                {
                    ...
                    float targetRotation = 0.25f * RotationMultiplier;
                    targetRotation = Random.Range(targetRotation, targetRotation + rotationRandomNess);

                    if (pause <= 0f)
                    {
                        if (rotationUp)
                        {
                            //r = Mathf.Lerp(fRotation, targetRotation, Time.deltaTime);
                            //rotation = Quaternion.Euler(0, r, r);


                            if (fRotation < targetRotation)
                            { 
                                fRotation += rotationSpeed;
                                fRotation += Time.deltaTime;
                                rotation = Quaternion.Euler(0, fRotation, fRotation);
                            }
                            else
                            {
                                rotationUp = false;
